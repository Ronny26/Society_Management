...
<body>
	...
	<header>
		<nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top">
			<a class="navbar-brand" href="/logged-home"><img height=30px width=30px
		...
		<table class="table table-striped table-bordered">
			<thead class="thead-dark">
				<tr>
					...
					<th>Maintainance Bill Status</th>
					<th>Water Bill</th>
			...
			<tbody>
				...
				<c:forEach items="${billdue}" var="bill">
					<tr>
						<td>${bill.id}</td>
						<td>${bill.elecBill}</td>
						<td>${bill.elecStat}</td>
						...
						<td>${bill.watBill}</td>
						<td>${bill.watStat}</td>
						<td><a href="/editbilldue/${bill.id}"><button type="submit" class="btn btn-primary">Edit</button></a></td>
					</tr>
				</c:forEach>
...
$('.nav-link').each(function(){
    if ($(this).prop('href') == window.location.href) {
        $(this).addClass('activee'); $(this).parents('li').addClass('activee');
    }
});
